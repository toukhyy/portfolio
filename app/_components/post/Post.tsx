...
export function FeaturedCard({ post }: { post: Meta }) {
  ...
  return (
    <div className="flex w-full flex-col items-center justify-between overflow-hidden rounded-md bg-cream-200 text-cream-900 shadow-sm outline outline-offset-4 outline-transparent duration-300 hover:outline-cream-500 md:h-[50vh] md:flex-row md:items-stretch lg:h-[70vh]">
        ...
        <div className="flex h-full flex-col justify-between gap-2">
          <div>
            <Link
              ...
              className="mb-1.5 inline-block text-xl font-semibold md:mb-4 md:text-3xl lg:text-4xl"
            >
              {title}
            </Link>
              ...
              <div>
                {translations.map((sub) => (
                  <Link
                    href={`blog/${sub}/${slug}`}
                    key={sub}
                    className="mr-2 inline-block rounded bg-cream-400 px-2 py-0.5 text-xs duration-300 hover:bg-cream-500 md:text-sm"
                  >
  ...
  return (
    <li className="w-72 overflow-hidden rounded-md text-cream-900 outline outline-2 outline-offset-4 outline-transparent duration-300 hover:bg-cream-100 hover:outline-cream-500">
      ...
      <div className="rounded-bl-lg rounded-br-lg border-b-2 border-l-2 border-r-2 border-cream-400 px-4 pt-2">
        ...
        <Link
          href={generateBlogPath(slug, post.translations)}
          className="mb-6 inline-block font-semibold"
        >
          {title}
        </Link>
          ...
          <div className="text-xs">
            {post.translations.map((sub) => (
              <Link
                ...
                key={sub}
                className="mr-1.5 inline-block rounded bg-cream-300 px-2 py-0.5 font-normal duration-300 hover:bg-cream-500"
              >
                {completeTranslations[sub]}
              </Link>
            ))}
